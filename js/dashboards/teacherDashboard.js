...
function TeacherDashboard({userData,onLogout,onUpdate}){
  ...
  const TABS=[{k:'analysis',icon:'📊',lbl:'학생관리'},{k:'worksheets',icon:'📝',lbl:'학습지'},{k:'notices',icon:'📢',lbl:'공지/숙제'}];
...
function StudentDetail({student,onBack,folders,customFolders,onAssignFolder,onClearFolder,onDeleteStudent}){
  ...
  return(<div className="p-4 pb-36 space-y-4">
    ...
    {(customFolders||[]).length>0&&(()=>{
      const myFolder=(customFolders||[]).find(f=>f.ids?.includes(student.id));
      return(<div className="bg-white rounded-2xl p-3 shadow-sm border border-gray-100 flex items-center gap-2 flex-wrap">
        <span className="text-xs font-bold text-gray-500">📁 폴더:</span>
        {myFolder
          ?<span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-lg">{myFolder.name}</span>
          :<span className="text-xs text-gray-400">미배정</span>}
        {myFolder&&<button onClick={()=>onClearFolder&&onClearFolder(student.id)} className="text-xs px-2 py-0.5 bg-gray-100 text-gray-500 rounded-lg font-bold">폴더 해제</button>}
        {(customFolders||[]).filter(f=>f.id!==myFolder?.id).map(f=>(
          <button key={f.id} onClick={()=>onAssignFolder&&onAssignFolder(f.id,student.id)}
            className="text-xs px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded-lg font-bold border border-indigo-100">📁 {f.name}</button>
        ))}
        <button onClick={()=>{if(onDeleteStudent)onDeleteStudent(student);}} className="ml-auto text-xs px-2 py-0.5 bg-red-50 text-red-500 rounded-lg font-bold">🗑️ 학생 삭제</button>
      </div>);
    })()}
