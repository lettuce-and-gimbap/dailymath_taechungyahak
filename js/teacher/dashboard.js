...
function TeacherDashboard({userData,onLogout,onUpdate,onSwitchUser}){
  ...
  return(<div className="teacher-ui flex flex-col min-h-screen max-w-2xl mx-auto bg-gray-50" style={{overflowX:'hidden',width:'100%',maxWidth:'100vw'}}>
    <header className="bg-indigo-700 text-white px-4 py-3 flex items-center gap-2 sticky top-0 z-20 shadow-md">
      ...
      <h1 className="text-lg font-black flex-1 min-w-0 truncate">선생님 모드</h1>
      <span className="text-sm font-bold opacity-80 truncate max-w-[64px] flex-shrink">{userData.name}</span>
      <div className="flex items-center gap-1.5 flex-shrink-0">
        {switchTarget&&<button onClick={()=>onSwitchUser(switchTarget)} title={`${switchTarget} 계정으로 전환`}
          className="flex items-center text-xs font-black px-2 py-1.5 rounded-lg bg-white/15 text-white active:scale-95 transition-transform whitespace-nowrap">
          🧑‍🎓 전환
        </button>}
        <DarkToggle/>
        <button onClick={onLogout} className="text-xs text-white/70 font-bold px-2 py-1 rounded-lg bg-white/10 whitespace-nowrap">로그아웃</button>
      </div>
    </header>
